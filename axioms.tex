...
\begin{definition}
A subcategory $\cat{C}\subseteq \Top{2}$ is \defn{admissible for homology} if the following apply:
\begin{enumerate}[(a)]
\item $C$ contains all points in $\mathbf{Top}$. In the language of category theory, $\cat{C}$ contains all \defn{final objects} in $\mathbf{Top}$, that is, all objects $\bullet\in\mathbf{Top}$ with the property that there is exactly one morphism $f:X\rightarrow \bullet$ for every $X\in \mathbf{Top}$.
...
\end{remark}
The extra dimension axiom is what defines an ordinary homology theory as opposed to a (general) homology theory. It is essential in our study. The choice $H_0=G$, called a \defn{choice of coefficients} distinguishes homology theories from each other.
...
\begin{definition}
For non-empty $X$, $\tilde{H}_nX:=ker(p^*:H_nX\rightarrow H_n\bullet)$ where $p^*$ is induced by the map $p:X\rightarrow \bullet$. $\tilde{H}_nX$ is called the \defn{$n$-th reduced homology group} of $X$.
\end{definition}
