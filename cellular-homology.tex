...
\begin{example}
The torus $T^2$ has a cellular structure composed of 1 0-cell, 2 1-cells and 1 2-cell. We can use the familiar identification of $T^2$ as a quotient of the square, where the two $1$-cells $a$ and $b$ have their endpoints glued to a single $0$-cell, as the corners are identified to a single point. The gluing map $\phi:S^1\rightarrow X^1$ is then the concatenation $a\cdot b\cdot -a \cdot -b$, where we are thinking of $a$ and $b$ as paths. The Klein bottle, which also arises as a quotient of the square, can be defined in a similar way.
